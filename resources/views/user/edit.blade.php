        ...
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ url('/user/' . $user->user_id) }}" class="formhorizontal"> @csrf
                    {!! method_field('PUT') !!} <!-- add this line for the editing process that requires the PUT method -->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <input type="password" class="form-control" id="password" name="password"> @error('password')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @else
                                <small class="form-text text-muted">Ignore (do not fill in) if you don't want to change the
                                    user.</small password>
                            @enderror
                        </div>
                    </div>
